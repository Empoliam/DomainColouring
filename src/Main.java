...
public class Main {
	...
	static public double XRES = 4000;
	static public double YRES = 4000;
	...
	public static void main(String[] args) {
		...
		for(int x = 0; x < XRES; x ++) {
			...
			for(int y = 0; y < YRES; y++) {
				...
				if(ZGRID) {
					if(Math.abs(Z.r)%ZGRIDDIV < 0.01)	output.setRGB(x, (int)YRES-1-y, 0);		
					else if(Math.abs(Z.i)%ZGRIDDIV < 0.01)	output.setRGB(x, (int)YRES-1-y, 0);	
				}

				if(WGRID) {
					double modr = Math.abs(W.r)%WGRIDDIV;
					double modi = Math.abs(W.i)%WGRIDDIV;
					if(modr < WGRIDTOLERANCE)	output.setRGB(x, (int)YRES-1-y, 0);		
					else if(modi < WGRIDTOLERANCE) output.setRGB(x, (int)YRES-1-y, 0);	
				}
